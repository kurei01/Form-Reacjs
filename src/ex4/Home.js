...
export class Home extends Component {
  ...
  state = { selectedProduct: null, cart: [], totalQuantity: 0 };

  selectProduct = (prodFromItem) => {
    this.setState({ selectedProduct: prodFromItem });
  };
  addToCart = (prod) => {
    ...
    let cloneQuatity = this.state.totalQuantity + 1;
    this.setState({ cart: cloneCart, totalQuantity: cloneQuatity });
  };
  deleteCartItem = (id) => {
    const cloneCart = [...this.state.cart];
    const foundIndex = cloneCart.findIndex((item) => item.product.id === id);
    let cloneQuantity =
      this.state.totalQuantity - cloneCart[foundIndex].quantity;
    cloneCart.splice(foundIndex, 1);
    // console.log(this.state.cart[foundIndex].quantity);
    this.setState({ cart: cloneCart, totalQuantity: cloneQuantity });
  };
  subCart = (id) => {
    const cloneCart = [...this.state.cart];
    const foundIndex = cloneCart.findIndex((item) => item.product.id === id);
    if (cloneCart[foundIndex].quantity > 1) cloneCart[foundIndex].quantity--;
    else cloneCart.splice(foundIndex, 1);
    let cloneQuatity = this.state.totalQuantity - 1;
    this.setState({ cart: cloneCart, totalQuantity: cloneQuatity });
  };
  clearCart = () => {
